...
import { patchData } from '../utilis/fetchData'
...
const Profile = () => {
    ...
    const changeAvatar =  (e) =>{
        ...
        return dispatch({type:'NOTIFY',payload:{error:'Image format is inccorect'}})
         console.log(file);
        ...
        <section className='row text-secondary my-3'>
           <div className="col-md-4">
                ...
                <div className="form-group">
                    ...
                    <input type="text " name='email' defaultValue={auth.user.email}  className='form-control' placeholder='Your name' disabled={true} onChange={handleChange}/>
                </div>
                <div className="form-group">
                    <label htmlFor="password">New Password</label>
                    <input type="password" name='password' value={password}  className='form-control' placeholder='Your new password'  onChange={handleChange}/>
                </div>
                <div className="form-group">
                    <label htmlFor="cf_password">Confirm New Password</label>
                    <input type="password" name='cf_password' value={cf_password}  className='form-control' placeholder='Confirm new password'  onChange={handleChange}/>
                </div>
                      ...
                      <tbody>
                          {
                              orders.map(order=>(
                                <tr  key={order._id}>
                                    ...
                                    <td className='p-2'>
                                        {order.delivered ?  <i  className='fas fa-check text-success'></i> : <i  className='fas fa-times text-danger'></i> }
                                    </td>
                                    <td className='p-2'>
                                    {
                                    order.paid
